...
public class ReportApi {
    ...
    public ResponseEntity<String> createFile(@AuthenticationPrincipal User user, @PathVariable Long reportId, @RequestParam("file") MultipartFile file){
        return fileEntityService.createReportFile(user.getUserId(),file, ContentTypes.REPORT,reportId);
    }
    //сурагч нь өөрийн явуулсан тайлангаа харах
    @GetMapping
    public List<ResponseReport> getStudentReports(@AuthenticationPrincipal User user){
        return reportService.getStudentReport(user.getUserId());
    }
    //тайланг id-гаар нь авах
    @GetMapping("/{reportId}")
    public ResponseReport getReportById(@AuthenticationPrincipal User user, @PathVariable Long reportId){
    ...
    @PutMapping("/file/{reportId}")
    public ResponseEntity<String> updateFile(@AuthenticationPrincipal User user, @PathVariable Long reportId, @RequestParam("file") MultipartFile file){
        return fileEntityService.updateFile(user.getUserId(),reportId,file);
